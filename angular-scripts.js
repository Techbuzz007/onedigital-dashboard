...
app.controller('locationsCtrl', function ($scope, $http, $rootScope, $window) {
  ...
  $scope.setlocationQRModal = function(location){
    $scope.active_location = location;
  }
...
app.controller('accesspassCtrl', function ($scope, $http, $rootScope, $window) {
  ...
  $scope.$watch('currentAccessPassToEdit.connection.type', function () {
    if ($scope.currentAccessPassToEdit && $scope.currentAccessPassToEdit.connection.type) {
      $scope.isEditPermanent = $scope.visitortypes.filter(function (e) {
        return e.url == $scope.currentAccessPassToEdit.connection.type && e.isPermanent == true;
      }).length > 0;
    }
  });
  ...
  $scope.resetForm = function () {
    ...
    $scope.child.resetData();
    $scope.child.recurrenceRule = null;
    $scope.parent = {dateFrom:'', dateTo:'',timeFrom:'', timeTo:''};
  }
  ...
  $scope.savePass = function () {
    ...
    if ($scope.name == null || $scope.name == '') {
    ...
    } else if (!$scope.isPermanent && ($scope.parent.dateFrom == null || $scope.parent.dateFrom == '')) {
      Swal.fire({
        text: 'You need to specify a dateFrom.',
        confirmButtonText: 'OK'
      });
    }
    else if (!$scope.isPermanent && $scope.parent.dateTo == $scope.parent.dateFrom) {
      Swal.fire({
        text: 'Starting date is the same as ending date.',
        confirmButtonText: 'OK'
      });
    }
    else if (!$scope.isPermanent && ($scope.parent.dateTo == null || $scope.parent.dateTo == '')) {
      Swal.fire({
      ...
      $scope.child.calculateRRule();
      var rule = $scope.child.recurrenceRule;
      var dateFrom = $scope.parent.dateFrom ? moment($scope.parent.dateFrom).toDate() : null;
      var dateTo   = $scope.parent.dateTo ? moment($scope.parent.dateTo).toDate() : null;
      rule.options.dtstart      = dateFrom;
      rule.options.until        = dateTo;
      rule.origOptions.dtstart  = dateFrom;
      rule.origOptions.until    = dateTo;
      ...
      newAccessPassfd.append('recurrance_str', rule.toString());
      newAccessPassfd.append('isEnable', true);
      if ($scope.email) {
        newAccessPassfd.append('email', $scope.email);
      }
      if ($scope.parent.dateFrom) {
        newAccessPassfd.append('timedatefrom', $scope.parent.dateFrom.toISOString());
      }
      if ($scope.parent.dateTo) {
        newAccessPassfd.append('timedateto', $scope.parent.dateTo.toISOString());
      }
      if (profile_img) {
      ...
      if ($scope.parent.timeFrom) {
        newAccessPassfd.append('time_from', moment.utc($scope.parent.timeFrom).format());
      }
      if ($scope.parent.timeTo) {
        newAccessPassfd.append('time_to', moment.utc($scope.parent.timeTo).format());
      }
  ...
  $scope.editPass = function () {
    ...
    var editAccessPassfd = new FormData();
    editAccessPassfd.append('name', $scope.currentAccessPassToEdit.connection.name);
    editAccessPassfd.append('phone', $scope.currentAccessPassToEdit.connection.phone);
    editAccessPassfd.append('vehicle_number', '[' + tempVehicles.toString() + ']');
    editAccessPassfd.append('type', $scope.currentAccessPassToEdit.connection.type);
    editAccessPassfd.append('residence_area', $scope.currentAccessPassToEdit.connection.residence_area);
    editAccessPassfd.append('number_of_guests', $scope.currentAccessPassToEdit.connection.number_of_guests);
    editAccessPassfd.append('isPermanent', $scope.isEditPermanent);
    if(profile_img){
      editAccessPassfd.append('profile_image', profile_img);
    }
    editAccessPassfd.append('email', $scope.currentAccessPassToEdit.connection.email);
    editAccessPassfd.append('company', $scope.currentAccessPassToEdit.connection.company);
    editAccessPassfd.append('comment', $scope.currentAccessPassToEdit.connection.comment);
    editAccessPassfd.append('isEnable', $scope.currentAccessPassToEdit.connection.isEnable);
      $http({
        method: 'PUT',
        url: $scope.currentAccessPassToEdit.connection.url,
        data: editAccessPassfd,
        transformRequest: angular.identity,
        headers: { 'Content-Type': undefined }
    }).then(function (resp) {
      Swal.fire({
...
app.controller('connectionsCtrl', function ($scope, $http, $rootScope, $window) {
  ...
  $scope.approveResident = function(){
    ...
    $scope.setConnectionStatus(currentConnection.url, 'approve', currentConnection.email, currentConnection.passcode, 
                              currentConnection.push_id, currentConnection,
                              rule.toString(),
                              $scope.parent.timeFrom ? moment.utc($scope.parent.timeFrom).format() : null,
                              $scope.parent.timeTo   ? moment.utc($scope.parent.timeTo).format()   : null );
  }
  ...
  $scope.setConResToPrint = function(residentPrintModal){
    var innerContents = `<div style="display:flex;justify-content:center"><div><h3>Resident's QR code</h3><img src='` + residentPrintModal.res_area.qr_code + `' style="margin-bottom: -100px" />
                          <h3>Resident's details</h3>
                          <ul>
                          <li> <b>Name: </b>`+ residentPrintModal.name +  `</li>
                          <li> <b>Phone: </b>` + residentPrintModal.phone +  `</li>
                          <li> <b>Address: </b>` + residentPrintModal.address +  `</li>
                          </ul></div></div>`;
    var popupWinindow = window.open('', '_blank', 'width=600,height=700,scrollbars=no,menubar=no,toolbar=no,location=no,status=no,titlebar=no');
    popupWinindow.document.open();
    popupWinindow.document.write('<html><head><link rel="stylesheet" type="text/css" href="style.css" /></head><body onload="window.print()">' + innerContents + '</html>');
